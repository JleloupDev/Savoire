...
/** In-memory stand-in for EdgeSyncHub: introduces peers, forwards frames. */
class FakeRelayServer {
  private readonly conns = new Map<string, FakeServerConnection>()
  private next = 0

  attach(): FakeServerConnection {
    const conn = new FakeServerConnection(this, `conn-${this.next++}`)
    return conn
  }
...
class FakeServerConnection implements RelayConnection {
