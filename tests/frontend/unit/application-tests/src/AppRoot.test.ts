...
import type { IVaultsBackend, IVaultSyncSessionFactory, IVaultSyncSession } from '@savoire/application'
...
function makeSession(): IVaultSyncSession {
  return {
    directory: {
      getAll: () => [], getById: () => undefined, add: vi.fn(), remove: vi.fn(), rename: vi.fn(),
      addFolder: vi.fn(), removeFolder: vi.fn(), getFolders: () => [],
      encodeFullState: () => new Uint8Array(), applyUpdate: vi.fn(),
      onLocalUpdate: vi.fn(() => () => {}), onChange: vi.fn(() => () => {}), dispose: vi.fn(),
    },
...
function makeVaultSyncSessionFactory(): IVaultSyncSessionFactory {
...
describe('AppRoot (integration)', () => {
  it('exposes api with all four service namespaces', () => {
    const root = new AppRoot({
      backend: makeBackend(),
      vaultSyncSessionFactory: makeVaultSyncSessionFactory(),
  ...
  it('api.vaults.list delegates to backend', async () => {
    ...
    const root = new AppRoot({ backend, vaultSyncSessionFactory: makeVaultSyncSessionFactory(), documentStore: makeDocumentStore() })
  ...
  it('api.documents.getActiveClient returns undefined before activation', () => {
    const root = new AppRoot({
      backend: makeBackend(), vaultSyncSessionFactory: makeVaultSyncSessionFactory(), documentStore: makeDocumentStore(),
    })
  ...
  it('api.documentSession.close is callable', () => {
    ...
    const root = new AppRoot({ backend: makeBackend(), vaultSyncSessionFactory: makeVaultSyncSessionFactory(), documentStore: store })
