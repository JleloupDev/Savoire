...
public class SharingApiTests : IClassFixture<AppFactory>, IAsyncLifetime
{
    ...
    [Fact]
    public async Task POST_document_link_Creates_ShareLink_For_Document()
    {
        // Create document in vault
        var docResp = await _owner.PostAsJsonAsync(
            $"/api/v1/vaults/{_vaultId}/documents",
            new { path = "shared-note.md", content = "# Hello" });
        ...
        var resp = await _owner.PostAsJsonAsync(
            ...
            new { permission = "write", expiresAt = (DateTime?)null });

        resp.StatusCode.Should().Be(HttpStatusCode.OK);
        var dto = await resp.Content.ReadFromJsonAsync<ShareLinkDto>();
        dto!.ResourceType.Should().Be("document");
        dto.ResourceId.Should().Be(doc.Id);
        dto.Permission.Should().Be("write");
    }

    [Fact]
    public async Task GET_share_access_ForDocumentLink_Returns_JWT_WithDocumentScope()
    {
        // Create document
        var docResp = await _owner.PostAsJsonAsync(
            $"/api/v1/vaults/{_vaultId}/documents",
            new { path = "scoped-note.md", content = "# Scoped" });
        var doc = await docResp.Content.ReadFromJsonAsync<DocumentDto>();

        // Create link
        var linkResp = await _owner.PostAsJsonAsync(
            $"/api/v1/documents/{doc!.Id}/sharing/links",
            new { permission = "write", expiresAt = DateTime.UtcNow.AddHours(1) });
