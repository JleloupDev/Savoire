...
//
// DECISION: Mermaid choisi (vs PlantUML) car JS pur, pas de serveur Java requis.
// renderClient est synchrone (contrat BlockSpec) — on retourne un div placeholder
// et on le peuple de façon asynchrone après le render Mermaid.
...
const plugin = {
    ...
    async onload(api) {
        api.blocks.register({
            ...
            detect: (text) => FENCE_RE.test(text.trim()) || CALLOUT_RE.test(text),
            deserialize: (raw) => {
