...
import type { BacklinksIndexContributor, BacklinkEntry } from './BacklinksIndexContributor'
...
function BacklinksPanel({
  ...
  ctx: ViewContext
  contributor: BacklinksIndexContributor
}) {
  const { workspace } = ctx
  ...
  return (
    <div style={T.panel}>
      ...
      <div style={T.body}>
        {!currentPath && (
          <div style={T.empty}>Ouvrir un document pour voir ses backlinks.</div>
        )}
        {currentPath && backlinks.length === 0 && (
          <div style={T.empty}>Aucun document ne pointe vers celui-ci.</div>
        )}
        {backlinks.map(bl => (
          <div
            key={bl.docId}
...
export class BacklinksWidget implements Widget {
  constructor(
    ...
    private readonly contributor: BacklinksIndexContributor,
  ) {}
